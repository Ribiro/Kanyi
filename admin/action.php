...
if (isset($_POST['editProduct'])){
    ...
    $sql = "UPDATE products SET `category_id`= '$category',`product_name` = '$name',`price` = '$price' WHERE product_id = '$id' ";
    $query = mysqli_query($con,$sql);
...
if (isset($_POST['editvendor'])) {
    ...
    if (empty($first) || empty($last) || empty($email) || empty($password) || empty($repassword) || empty($address) || empty($contact)) {
        ...
        $sql = "UPDATE vendor SET `fname`= '$first',`lname` = '$last',`email` = '$email',`password` = '$password',`address`='$address',`contact`='$contact' WHERE vendor_id = '$id' ";
        $run_query = mysqli_query($con, $sql);
        if ($run_query) {
            $_SESSION['success'] = 'Vendor updated successfully';
            header('location:vendors.php');
        } else {
            $_SESSION['error'] = 'Vendor not updated successfully';
            header('location:vendors.php');
        }
    }
}
...
if (isset($_POST['delvendor'])){
    ...
    if ($query){
        ...
        $_SESSION['error'] = 'Vendor not deleted';
        header('location:vendors.php');
    }
}